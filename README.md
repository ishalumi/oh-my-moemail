<p align="center">
  <img src="public/icons/icon-192x192.png" alt="Oh My MoeMail Logo" width="100" height="100">
  <h1 align="center">Oh My MoeMail</h1>
</p>
...
- 📂 **邮箱分类**：前端 Tab 切换全部/永久/时效邮箱，一目了然
- 🌐 **CF 域名导入**：一键从 Cloudflare 账号批量导入域名，多选添加
- 🔀 **子域名管理**：输入前缀自动派生子域，自动创建 DNS 记录 + Email Routing
- 🔍 **筛选与搜索**：支持按域名筛选、关键词搜索邮箱地址
- 🗑️ **真删除清理**：过期邮箱物理删除，永久邮箱自动保留最近 100 条消息
- 📐 **API 字段优化**：消息字段统一为 `sender/recipient/text`，DB 到 API 零映射
- 🗃️ **数据库优化**：域名数据从 KV 迁移到 D1 `domains` 表，支持原生域/子域分类
...
1. 克隆仓库：
```bash
git clone https://github.com/ishalumi/oh-my-moemail.git
cd oh-my-moemail
```
...
在 MoeMail 个人中心页面的**域名管理**模块中管理邮箱域名：

- **从 CF 导入**：一键导入 Cloudflare 账号下的所有域名，多选批量添加
- **手动添加**：支持原生域和子域，子域输入前缀自动拼接父域名
...
MoeMail 同时提供 [MCP](https://modelcontextprotocol.io) 服务器，任何支持 MCP 的客户端（Claude Desktop、Cursor、Cline 等）都能直接获得临时邮箱工具，无需调用 CLI。
...
| `read_message` | 读取邮件完整正文（text/HTML） |
| `wait_for_email` | 轮询等待新邮件（有上限；超时返回 `status: "timeout"` 以便重试） |
...
在 MCP 客户端配置中添加服务器（如 Claude Desktop 的 `claude_desktop_config.json`），凭证通过环境变量传入：
